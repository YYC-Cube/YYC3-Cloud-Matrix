  ...
  return {
    ...
    configureServer(server: ViteDevServer) {
      ...
      server.httpServer?.on('upgrade', (req, socket, head) => {
        ...
        if (url.pathname === '/api/v2/terminal/ws') {
      ...
      // RESTful API: 创建会话
      server.middlewares.use('/api/v2/terminal/create', (req, res) => {
        if (req.method !== 'POST') {
      ...
      server.middlewares.use('/api/v2/terminal/exec', async (req, res) => {
        if (req.method !== 'POST') {
          res.writeHead(405, { 'Content-Type': 'text/plain' })
          res.end('Method Not Allowed')
          return
      ...
      // RESTful API: 终止会话
      server.middlewares.use('/api/v2/terminal/kill', (req, res) => {
        if (req.method !== 'POST') {
          res.writeHead(405)
          res.end('Method Not Allowed')
          return
      ...
      // RESTful API: 会话状态
      server.middlewares.use('/api/v2/terminal/status', (req, res) => {
        if (req.method !== 'GET') {
          res.writeHead(405)
          res.end('Method Not Allowed')
          return
