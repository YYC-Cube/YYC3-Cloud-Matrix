 ...
 * @description: 真实终端命令执行 API 中间件 (V1 - 已弃用)
 *              所有新开发请使用终端 API V2 (`/api/v2/terminal/`)
 *              V1 将在 2026-09-01 移除，请尽快迁移
...
import { validateExecRequest, validateSessionId, safeJSONParse, validateWSMessage, formatValidationResponse } from './input-validator';
...
export function createTerminalAPI(): Plugin {
  ...
  return {
    ...
    configureServer(viteServer) {
        ...
        ws.on('message', (data) => {
          try {
            ...
            if (!parseResult.success) {
              ws.send(JSON.stringify({ type: 'error', message: parseResult.error }))
              return
            }
            ...
            if (!validation.valid) {
              ws.send(JSON.stringify({
                type: 'error',
                message: formatValidationResponse(validation).error,
      ...
      // 创建新会话
      viteServer.middlewares.use('/api/v1/terminal/create', (req, res) => {
        if (req.method !== 'POST') {
      ...
      // 执行命令
      viteServer.middlewares.use('/api/v1/terminal/exec', async (req, res) => {
        if (req.method !== 'POST') {
          res.writeHead(405, { 'Content-Type': 'text/plain' })
          res.end('Method Not Allowed')
          return
        ...
        req.on('end', () => {
          try {
            ...
            if (!parseResult.success) {
              jsonResponse(res, 400, { error: parseResult.error })
              return
            }

            const { sessionId, command, cwd } = parseResult.data

            // 输入验证
            const validation = validateExecRequest({ sessionId, command, cwd })
            if (!validation.valid) {
              jsonResponse(res, 400, formatValidationResponse(validation))
              return
            }
      ...
      // 终止会话
      viteServer.middlewares.use('/api/v1/terminal/kill', (req, res) => {
        if (req.method !== 'POST') {
          res.writeHead(405, { 'Content-Type': 'text/plain' })
          res.end('Method Not Allowed')
          return
      ...
      // 获取会话状态
      viteServer.middlewares.use('/api/v1/terminal/status', (req, res) => {
        if (req.method !== 'GET') {
          res.writeHead(405, { 'Content-Type': 'text/plain' })
          res.end('Method Not Allowed')
          return
      ...
      // ================================================================
      // 兼容层：旧版 /api/terminal/* → 重定向至 /api/v2/terminal/*
      // 2026-09-01 后将移除此兼容层
      // ================================================================
      const legacyRedirect = (req: any, res: any, next: any) => {
        const url = req.url || ''
        const legacyMatch = url.match(/^\/api\/terminal\/(.*)/)
        if (legacyMatch) {
          const newPath = `/api/v2/terminal/${legacyMatch[1]}`
          logger.warn(`[API Compat] 旧版路径重定向: ${url} → ${newPath}`)
          res.writeHead(308, {
            'Content-Type': 'application/json',
            'Location': newPath,
            'X-API-Deprecated': 'true',
            'X-API-Deprecation-Date': '2026-09-01',
            'X-API-Migration-URL': 'https://docs.yyc3.dev/api/v2/terminal',
          })
          res.end(JSON.stringify({
            error: 'API 已版本化，请使用新路径',
            deprecated: true,
            redirectTo: newPath,
            migrationGuide: 'https://docs.yyc3.dev/api/v2/terminal',
      ...
      viteServer.middlewares.use(legacyRedirect)
