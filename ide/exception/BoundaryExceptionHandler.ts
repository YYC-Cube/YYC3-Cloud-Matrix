...
export class BoundaryExceptionHandler {
  ...
  catchException(
    ...
    // Sentry 上报
    if (this.config.enableSentry && sentryService.isInitialized()) {
      this.reportToSentry(record);
    }
  ...
  private addRecord(record: ExceptionRecord): void {
    ...
    if (this.records.size > (this.config.maxRecords ?? 100)) {
      let deleted = false;
      this.records.forEach((rec, id) => {
        if (!deleted && rec.resolved) {
  ...
  private logException(record: ExceptionRecord): void {
    ...
    switch (record.severity) {
      ...
      case "high":
        ...
        if (record.error) {
          logger.error(record.error.message, record.error.stack);
        }
        break;
      case "medium":
   ...
   * 上报异常至 Sentry
   */
  private reportToSentry(record: ExceptionRecord): void {
    try {
      const severity = this.mapToSentrySeverity(record.severity);

      if (record.error) {
        sentryService.captureError(record.error, {
          tags: {
        ...
        sentryService.captureMessage(
          `[${record.type.toUpperCase()}] ${record.message}`,
          severity
        );
      }
    } catch {
   ...
   * 映射严重级别到 Sentry 级别
   */
  private mapToSentrySeverity(severity: ExceptionSeverity): any {
    switch (severity) {
      case 'critical':
        return 'fatal';
      case 'high':
