...
export async function loadFile(
  ...
  const db = await getDB();
  const file = await db.get(STORE_FILES, `${projectId}/${path}`) as StoredFile | undefined;
...
export async function loadAllFiles(
  ...
  const db = await getDB();
  const files = (await db.getAll(STORE_FILES)) as StoredFile[];
...
export async function deleteAllFiles(projectId: string): Promise<void> {
  ...
  for (const file of files) {
    if (file.projectId === projectId) {
      tx.objectStore(STORE_FILES).delete(file.path);
    }
  }
  await tx.done;
}
...
export async function loadProject(id: string): Promise<StoredProject | null> {
  const db = await getDB();
  const project = await db.get(STORE_PROJECTS, id) as StoredProject | undefined;
  return project ?? null;
}

export async function listProjects(): Promise<StoredProject[]> {
  const db = await getDB();
  const projects = (await db.getAll(STORE_PROJECTS)) as StoredProject[];
  return projects.sort((a: StoredProject, b: StoredProject) => b.updatedAt - a.updatedAt);
}
...
export async function loadSnapshot(id: string): Promise<StoredSnapshot | null> {
  const db = await getDB();
  const snapshot = await db.get(STORE_SNAPSHOTS, id) as StoredSnapshot | undefined;
...
export async function listSnapshots(
  ...
  const db = await getDB();
  const allSnapshots = (await db.getAll(STORE_SNAPSHOTS)) as StoredSnapshot[];
  const snapshots = allSnapshots.filter(s => s.projectId === projectId);
  return snapshots.sort((a: StoredSnapshot, b: StoredSnapshot) => b.createdAt - a.createdAt);
}
...
export function debouncedSaveFiles(
  ...
  saveTimer = setTimeout(() => {
    saveFiles(projectId, files).catch((err) => logger.error('Failed to save files', err));
  }, DEBOUNCE_MS);
}
