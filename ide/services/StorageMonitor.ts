...
class StorageMonitorService {
  ...
  async getStorageUsage(): Promise<StorageUsage> {
    ...
    try {
      ...
      logger.error("[StorageMonitor] Error calculating localStorage usage:", e);
    }
    ...
    try {
      ...
      indexedDBUsage = files.reduce((sum: number, file: StoredFile) => {
      ...
      logger.error("[StorageMonitor] Error calculating IndexedDB usage:", e);
    }
  ...
  async getStorageBreakdown(): Promise<StorageBreakdown> {
    ...
    try {
      ...
      breakdown.files.size = files.reduce((sum: number, file: StoredFile) => {
        return sum + (file.content?.length || 0) * 2;
      }, 0);
