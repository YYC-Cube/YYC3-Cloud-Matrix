...
phase: testing
related_docs: YYC3-代码语法-测试核验.md,YYC3-功能逻辑-验收标准.md,YYC3-测试用例-审核验收.md,YYC3-组件测试-验收标准.md,YYC3-单元框架-审核验收.md,YYC3-闭环验证-验收标准.md,YYC3-数据调取-拓展建议.md,YYC3-多维分析-安全加固.md,YYC3-深度探索-高级功能.md,YYC3-深度审核-性能优化.md,YYC3-智能验收-AI赋能.md,YYC3-文档体系-闭环审核.md,YYC3-现状审核-分析建议.md
---
...
function calculateTimeScore(metrics: TimeDimensionMetrics): {
  ...
  // 测试执行时间评分
  const totalTestTime = metrics.testExecutionTime.unit + metrics.testExecutionTime.integration + metrics.testExecutionTime.e2e;
  if (totalTestTime <= 120000) {
    totalScore += 100 * weights.testExecutionTime;
  } else if (totalTestTime <= 300000) {
    totalScore += 80 * weights.testExecutionTime;
    recommendations.push('测试执行时间偏长，建议优化测试用例或启用并行执行');
  } else {
    totalScore += 50 * weights.testExecutionTime;
    recommendations.push('严重警告：测试执行时间超过5分钟');
  }

  // 验收周期评分
  if (metrics.acceptanceCycle.durationHours <= 8) {
    totalScore += 100 * weights.acceptanceCycle;
  } else if (metrics.acceptanceCycle.durationHours <= 24) {
    totalScore += 75 * weights.acceptanceCycle;
    recommendations.push('验收周期较长，建议优化流程或增加自动化');
  } else {
    totalScore += 50 * weights.acceptanceCycle;
    recommendations.push('验收周期过长，需要重新评估流程效率');
  }

  // 缺陷修复时间评分
  const p0Ok = metrics.defectResolutionTime.p0.avg <= 4;
  const p1Ok = metrics.defectResolutionTime.p1.avg <= 24;
  const p2Ok = metrics.defectResolutionTime.p2.avg <= 72;
  const passCount = [p0Ok, p1Ok, p2Ok].filter(Boolean).length;

  if (passCount === 3) {
    totalScore += 100 * weights.defectResolutionTime;
  } else if (passCount >= 2) {
    totalScore += 75 * weights.defectResolutionTime;
    recommendations.push('部分优先级缺陷修复超时，需加强监控');
  } else {
    totalScore += 40 * weights.defectResolutionTime;
    recommendations.push('严重警告：缺陷修复严重超时，需立即改进');
  }
...
/**
 * 代码架构健康检查
 * 使用 madge 检测循环依赖、eslint 检查组件大小、bundle-analyzer 分析包体积
 */
const architectureHealthCheck = {
  ...
  checkCircularDependencies: async () => {
    ...
    if (circular.length > 0) {
      console.error('检测到循环依赖:', circular);
      throw new Error(`存在 ${circular.length} 个循环依赖，必须修复`);
    }
    return { passed: true, circularCount: 0 };
  },

  /** 检查组件文件大小，确保不超过 maxLines 行 */
    ...
    function scan(dir: string) {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          if (lines > maxLines) {
            violations.push(`${fullPath}: ${lines} 行 (超过 ${maxLines} 行限制)`);
          }
        }
      }
    }
  ...
  analyzeBundleSize: async () => {
    // 在 CI 环境中使用 stats.json 进行分析
    const statsPath = 'dist/stats.json';
    try {
      const fs = require('fs');
      if (!fs.existsSync(statsPath)) {
        return { passed: true, note: 'stats.json 未生成，跳过分析' };
      }
      const stats = JSON.parse(fs.readFileSync(statsPath, 'utf-8'));
      const mainBundleSize = stats.assets
        .filter((a: { name: string }) => a.name.endsWith('.js'))
        .reduce((sum: number, a: { size: number }) => sum + a.size, 0);
      const passed = mainBundleSize < 500 * 1024; // 500KB gzipped
      return { passed, mainBundleSize };
    } catch {
      return { passed: true, note: '无法读取构建产物，跳过分析' };
    }
  },
};
```
...
function calculateQualityScore(attributes: QualityAttributes): {
  ...
  // 以下辅助函数定义在各自模块中，此处仅展示调用逻辑
// import { calculateFunctionalityScore } from './functionality';
...
// import { calculateMaintainabilityScore } from './maintainability';
// import { calculateTestabilityScore } from './testability';
...
class ClosedLoopVerificationController {
  ...
  // 以下方法实现细节参考各子阶段文档：
  // - executeAcceptance(): 执行自动化测试 + 人工审查流程
  // - calculateOverallSummary(): 汇总各阶段评分
  // - compileDetails(): 编译详细结果
  // - evaluateFiveDimensions(): 评估五维指标
  // - generateRecommendations(): 生成改进建议
  private executeAcceptance(): any { return {}; }
  private calculateOverallSummary(): any { return {}; }
  private compileDetails(): any { return {}; }
  private evaluateFiveDimensions(): any { return {}; }
...
| 文档 | 说明 |
|-----|------|
| [YYC3-团队核心-五维驱动](../YYC3-团队核心-五维驱动.md) | 五维驱动理论详解 |
| [YYC3-团队规范开发标准](../YYC3-团队规范开发标准.md) | 开发规范与标准 |
...
| 版本 | 日期 | 作者 | 变更说明 |
|-----|------|------|---------|
| v2.0.0 | 2026-07-24 | YanYuCloudCube Team | 版本迭代同步 — 新增智能验收与AI赋能阶段（第11阶段）；完善五维评估框架 Zod 类型定义；补充闭环验证控制器；新增质量门禁 CI/CD 配置；修正分类体系与标签；补充元数据字段 |
