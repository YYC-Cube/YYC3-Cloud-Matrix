...
export type EmotionType = "happy" | "sad" | "anxious" | "confused" | "angry" | "neutral" | "excited" | "calm" | "relaxed";
...
class EmotionMusicBridgeClass {
  ...
  analyzeSentiment(text: string): number {
    ...
    CONFUSION_KEYWORDS.forEach((keyword) => {
      if (lowerText.includes(keyword)) {
        score -= 0.1;
      }
    });
  ...
  private updateEmotion(emotion: EmotionState): void {
    ...
    if (previousEmotion && previousEmotion.type !== emotion.type) {
      musicEventBus.emit({
        ...
        payload: {
          previousEmotion: previousEmotion.type,
...
export const emotionMusicBridge = new EmotionMusicBridgeClass();

export default emotionMusicBridge;
