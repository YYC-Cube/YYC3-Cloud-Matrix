...
export function CreateRuleModal({ open, onClose, onSubmit, editRule }: CreateRuleModalProps) {
  ...
  const updateThreshold = useCallback(
    (index: number, field: keyof AlertThreshold, value: unknown) => {
      setThresholds((prev) => prev.map((th, i) => (i === index ? { ...th, [field]: value } : th)));
    },
    [],
  );
  ...
  const addEscalation = useCallback(() => {
    setEscalation((prev) => {
      ...
      return [
        ...prev,
        { level: nextLevel, delayMinutes: nextLevel * 10, notifyChannels: ["dashboard"] },
      ];
    });
  }, []);
  ...
  const updateEscalation = useCallback((index: number, field: string, value: unknown) => {
    setEscalation((prev) => prev.map((esc, i) => (i === index ? { ...esc, [field]: value } : esc)));
  }, []);
      ...
      >
        ...
        <div className="flex items-center justify-between p-5 border-b border-[rgba(0,180,255,0.1)]">
          <h2
            ...
            style={{ fontSize: "0.95rem", fontFamily: "'Orbitron', sans-serif" }}
          >
        ...
        <div className="p-5 space-y-5">
          ...
          {error && (
            <div
              className="p-2.5 rounded-lg border border-[rgba(255,51,102,0.3)] bg-[rgba(255,51,102,0.08)] text-[#ff3366]"
              ...
              {SEVERITIES.map(({ value, color, icon: Icon }) => (
                ...
                >
                  <Icon
                    ...
                    style={{ color: severity === value ? color : "rgba(0,212,255,0.3)" }}
                  />
          ...
          <div>
            ...
            <div className="space-y-2">
              {thresholds.map((th, i) => (
                <div
                  key={i}
                  className="flex items-center gap-2 p-2.5 rounded-lg border border-[rgba(0,180,255,0.08)] bg-[rgba(0,180,255,0.02)]"
                >
                  <select
                    value={th.metric}
                  ...
                  >
                    {METRICS.map((m) => (
                      <option key={m.value} value={m.value} style={{ background: "#060e1f" }}>
                        {m.label}
                      </option>
                    ))}
                  </select>
                  ...
                  >
                    {CONDITIONS.map((c) => (
                      <option key={c.value} value={c.value} style={{ background: "#060e1f" }}>
                        {c.label}
                      </option>
                    ))}
                  </select>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-3 rounded-lg border border-[rgba(0,180,255,0.1)] bg-[rgba(0,180,255,0.02)]">
              <label className="flex items-center gap-2 mb-2 cursor-pointer">
                ...
                />
                <span style={{ fontSize: "0.72rem", color: "#e0f0ff" }}>
                  {t("alerts.enableAggregation")}
                </span>
              </label>
              {aggEnabled && (
                <div className="space-y-2">
                  <div>
                    ...
                    <input
                      ...
                      title={t("alerts.aggregationWindow")}
                      style={inputStyle}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      title={t("alerts.aggregationMaxGroup")}
                      style={inputStyle}
                    />
                  </div>
                </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              {escalation.map((esc, i) => (
                <div
                  key={i}
                  className="p-3 rounded-lg border border-[rgba(0,180,255,0.08)] bg-[rgba(0,180,255,0.02)]"
                >
                  <div className="flex items-center justify-between mb-2">
                    <span
                        ...
                        background:
                          esc.level === 3
                            ...
                            : esc.level === 2
                              ...
                              : "rgba(0,212,255,0.15)",
                        color:
                          esc.level === 3 ? "#ff3366" : esc.level === 2 ? "#ffaa00" : "#00d4ff",
                      }}
                    >
                      L{esc.level}
                    </span>
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-2">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateEscalation(i, "delayMinutes", Number(e.target.value))
                        }
                        title={t("alerts.delay")}
                        style={inputStyle}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          updateEscalation(i, "autoAction", e.target.value || undefined)
                        }
                        style={inputStyle}
                      ...
                      {CHANNEL_OPTIONS.map((ch) => (
                        <button
                          ...
                          style={{
                            ...
                            background: esc.notifyChannels.includes(ch)
                              ? "rgba(0,212,255,0.1)"
                              : "transparent",
                            color: esc.notifyChannels.includes(ch)
                              ? "#00d4ff"
              ...
              {AVAILABLE_NODES.map((node) => (
                <button
                  ...
                  style={{
                    ...
                    background: selectedNodes.includes(node)
                      ? "rgba(0,255,136,0.08)"
                      : "transparent",
