...
export function FamilyPhone() {
  ...
  // 通话中家人会说话
  useEffect(() => {
    if (callState !== "connected" || !callingMember) {
      return;
    }
    const phrases = [
  ...
  if (callState !== "idle") {
    ...
    return (
      <div
        ...
        style={{
          background: `radial-gradient(ellipse at top, ${m.color}08 0%, rgba(4,8,20,1) 50%, #020510 100%)`,
        }}
      >
        {/* 通话头部 */}
        <div className="flex-1 flex flex-col items-center justify-center px-4">
          ...
          <div className="relative mb-6">
            <div
              ...
              style={{
                ...
                boxShadow:
                  callState === "connected"
                    ? `0 0 40px ${m.color}30, 0 0 80px ${m.color}10`
                    : `0 0 20px ${m.color}15`,
          ...
          {/* 名字 + 号码 */}
          <h2 className="text-[#e0f0ff]" style={{ fontSize: "1.3rem" }}>
            {m.name}
          </h2>
          <p
            className="text-[rgba(224,240,255,0.4)] mt-1 font-mono"
          ...
          <div className="mt-3 flex items-center gap-2">
            {callState === "dialing" && (
              <span className="text-[rgba(224,240,255,0.4)]" style={{ fontSize: "0.78rem" }}>
                正在拨号...
              </span>
            )}
            {callState === "ringing" && (
            ...
            {callState === "ended" && (
              <span className="text-[rgba(224,240,255,0.3)]" style={{ fontSize: "0.78rem" }}>
                通话已结束
              </span>
            )}
          </div>
          ...
          {callState === "connected" && chatMessages.length > 0 && (
            <div className="mt-6 w-full max-w-sm space-y-2 px-4">
              {chatMessages.map((msg, i) => (
                ...
                >
                  <p
                    className="text-[rgba(224,240,255,0.7)]"
          ...
          <div className="flex items-center justify-center gap-6">
            {callState === "connected" && (
              <>
                ...
                >
                  {isMuted ? (
                    <MicOff className="w-5 h-5 text-[#ff3366]" />
                  ) : (
                    <Mic className="w-5 h-5 text-[rgba(224,240,255,0.6)]" />
                  )}
                </button>
                <button
                ...
                >
                  {isSpeaker ? (
                    ...
                    <VolumeX className="w-5 h-5 text-[rgba(224,240,255,0.6)]" />
                  )}
                </button>
              </>
            )}
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className="text-[rgba(224,240,255,0.25)]"
                        ...
                        <p
                          className="text-[rgba(224,240,255,0.4)] mt-0.5 font-mono"
                          style={{ fontSize: "0.7rem" }}
                        >
                          {m.phone}
                        </p>
                        <p
                          className="text-[rgba(224,240,255,0.3)] mt-0.5 truncate"
                          style={{ fontSize: "0.62rem" }}
                        >
                          {typeof m.personality === "string"
                            ? m.personality
                            : m.personality.description}
                        </p>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        <button
                          onClick={() => startCall(m)}
                          title={`呼叫${m.shortName}`}
                ...
                {members.map((m) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <p
                        className="text-[rgba(224,240,255,0.5)] truncate"
                        style={{ fontSize: "0.55rem" }}
                      >
                        {m.shortName}
                      </p>
                      <p
                        className="text-[rgba(224,240,255,0.25)] font-mono"
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span
                            style={{
                              fontSize: "0.82rem",
                              color: log.type === "missed" ? "#ff3366" : m.color,
                            }}
                          >
                            {m.name}
                          </span>
                        ...
                        <div className="flex items-center gap-3 mt-0.5">
                          ...
                          {log.duration && (
                            <span
                              className="text-[rgba(224,240,255,0.2)]"
                              style={{ fontSize: "0.62rem" }}
                            >
                              {log.duration}
                            </span>
