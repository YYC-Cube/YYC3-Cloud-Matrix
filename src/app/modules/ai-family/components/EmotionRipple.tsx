...
export type EmotionType = "happy" | "energetic" | "calm" | "sad" | "neutral";
...
export const EmotionRipple: React.FC<EmotionRippleProps> = ({
  ...
  const targetPaletteRef = useRef<EmotionPalette>(EMOTION_PALETTES.neutral);
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      if (isPlaying) {
        ...
        grad.addColorStop(
          0,
          `hsla(${palette.h}, ${palette.s}%, ${palette.l}%, ${0.06 + audioEnergy * 0.08})`
        );
      ...
      if (isPlaying) {
        ...
        if (frequencyData && frequencyData.length > 5 && now > beatCooldownRef.current) {
          ...
          if (bassRise > 0.12 && bassEnergy > 0.35) {
            beatCooldownRef.current = now + 120;
            for (let k = 0; k < 3; k++) {
              const palette = currentPaletteRef.current;
              ripplesRef.current.push({
      ...
      if (isPlaying) {
        if (audioEnergy > 0.65) {
          if (highEnergyStartRef.current === 0) {
            highEnergyStartRef.current = now;
          }
        ...
        if (isClimaxRef.current) {
          ...
          const climaxGrad = ctx.createRadialGradient(
            cx,
            cy,
            0,
            cx,
          ...
          climaxGrad.addColorStop(
            0,
            `hsla(${palette.h}, ${palette.s + 20}%, ${palette.l + 10}%, ${0.04 + audioEnergy * 0.06})`
          );
          climaxGrad.addColorStop(
            0.5,
            `hsla(${palette.h + 20}, ${palette.s}%, ${palette.l}%, ${0.02 + audioEnergy * 0.03})`
          );
  ...
  }, [isPlaying, audioEnergy, lerpColor, frequencyData]);
