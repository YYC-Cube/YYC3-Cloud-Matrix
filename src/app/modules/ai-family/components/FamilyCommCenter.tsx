...
import { AI_RESPONSES, SAMPLE_MESSAGES, hexToRgb, type FamilyMessage } from "./shared";
        ...
        return (
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg shrink-0 transition-all ${
              active ? "border" : "bg-white/[0.03] border border-transparent hover:bg-white/[0.06]"
            }`}
            style={{
              ...(active
                ? {
    ...
    <div className="flex items-start gap-2.5 group">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-1.5 mb-0.5">
          ...
          {to && (
            <>
              <span className="text-white/15" style={{ fontSize: "0.55rem" }}>
                to
              </span>
          ...
          {msg.to === "all" && (
            <span
              className="px-1 py-0.5 rounded text-white/30"
              style={{ fontSize: "0.5rem", background: "rgba(255,255,255,0.04)" }}
            >
              @全家
            </span>
          )}
        ...
        >
          <span className="text-white/60" style={{ fontSize: "0.75rem", lineHeight: "1.6" }}>
            {msg.content}
          </span>
        </div>
        {msg.read && (
...
function DateSeparator({ label }: { label: string }) {
  return (
    <div className="flex items-center gap-3 my-3">
      ...
      <span
        className="flex items-center gap-1 px-2 py-0.5 rounded bg-white/[0.03] text-white/25"
...
export function FamilyCommCenter() {
  ...
  const filteredMessages = useMemo(() => {
    const result = messages.filter((msg) => {
      if (filterMember !== "all") {
        if (msg.from !== filterMember && msg.to !== filterMember && msg.to !== "all") {
          return false;
        }
      }
    ...
    <div className="min-h-full pb-8">
      <div className="max-w-4xl mx-auto flex flex-col">
        ...
        <FadeIn>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-white/90" style={{ fontSize: "1.25rem" }}>
                  Family 通信中心
                </h1>
                <p className="text-white/40" style={{ fontSize: "0.7rem" }}>
                  {totalCount} 条消息 · {unreadCount} 条未读 ·{" "}
                  {members.filter((m) => m.status === "online").length} 人在线 · 本地持久化
                </p>
              </div>
            </div>
        ...
        <div className="flex items-center gap-1.5 my-3">
          ...
          {[
            { key: "all", label: "全部" },
            ...Object.entries(MSG_TYPE_CONFIG).map(([k, v]) => ({ key: k, label: v.label })),
          ].map((item) => (
            <button
              key={item.key}
        ...
        {/* Messages area */}
        <GlassCard
          className="flex-1 p-4 overflow-y-auto mb-4"
