...
export function DatabaseManager() {
  ...
  const executeInlineSQL = useCallback(
    async (sql: string): Promise<{ ok: boolean; error?: string; affectedRows?: number }> => {
    ...
    <div className="space-y-4">
      ...
      <div className={`grid gap-2 ${isMobile ? "grid-cols-2" : "grid-cols-5"}`}>
        {[
          ...
          <div
            key={s.label}
            className="px-3 py-2 rounded-xl bg-[rgba(0,40,80,0.12)] border border-[rgba(0,180,255,0.06)]"
          >
      ...
      <div className="flex items-center gap-1 flex-wrap">
        {tabs.map((tab) => (
          ...
          >
            ...
            {tab.count > 0 && (
              <span
                className="px-1.5 py-0.5 rounded-full bg-[rgba(0,212,255,0.1)]"
                style={{ fontSize: f.xs }}
              >
                {tab.count}
              </span>
        ...
        <div className="space-y-3">
          ...
          {showAddForm && (
            <GlassCard className="p-4">
              ...
              <div className={`grid gap-3 ${isMobile ? "grid-cols-1" : "grid-cols-2"}`}>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setFormData((d) => ({ ...d, database: e.target.value }))}
                ...
                <div className="col-span-full">
                  ...
                  <div className="relative mt-1">
                    ...
                    />
                    <button
                      onClick={() => setShowPassword((p) => ({ ...p, form: !p.form }))}
              ...
              <div className="flex justify-end gap-2 mt-4">
                <button
                  onClick={() => setShowAddForm(false)}
                  className="px-4 py-2 rounded-lg text-[rgba(0,212,255,0.4)]"
                  style={{ fontSize: f.sm }}
                >
                  取消
                </button>
          ...
          {db.connections.length === 0 ? (
            <GlassCard className="p-8 text-center">
              ...
              <p style={{ fontSize: f.xs, color: textDim }}>
                点击&ldquo;自动检测&rdquo;扫描本地服务，或手动&ldquo;新建连接&rdquo;
              </p>
            </GlassCard>
          ) : (
            <div className="space-y-2">
              {db.connections.map((conn) => {
                ...
                return (
                  <GlassCard
                    key={conn.id}
                    className={`p-4 ${isActive ? "ring-1 ring-[rgba(0,255,136,0.3)]" : ""}`}
                  >
                    <div className="flex items-center gap-3">
                      <div
                      ...
                      >
                        <span
                          style={{ color: meta.color, fontSize: f.sm, fontFamily: "monospace" }}
                        >
                          {meta.icon}
                        </span>
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        <p style={{ fontSize: f.xs, color: textDim }}>
                          ...
                          {conn.lastConnected &&
                            ` · 上次: ${new Date(conn.lastConnected).toLocaleString("zh-CN", { month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" })}`}
                        </p>
                      </div>
                      <div className="flex items-center gap-1.5">
                        ...
                        {conn.status === "connected" ? (
                          <button
                            onClick={() => db.disconnectDB(conn.id)}
                        ...
                        <button
                          onClick={() => db.removeConnection(conn.id)}
                          title="删除连接"
            ...
            <>
              <div className="flex items-center justify-between">
                <span style={{ fontSize: f.xs, color: textDim }}>
                  {db.tables.length} 个表 · 总计 {db.stats.totalTableRows.toLocaleString()} 行 ·{" "}
                  {formatBytes(db.stats.totalTableSize)}
                </span>
              </div>
              {db.tables.map((table) => (
                <GlassCard key={table.name} className="p-0 overflow-hidden">
                  ...
                  {expandedTable === table.name && (
                    <div className="px-4 pb-4 border-t border-[rgba(0,180,255,0.06)]">
                      {/* 列定义 */}
                      <p className="mt-3 mb-1" style={{ fontSize: f.xs, color: textDim }}>
                        列定义
                      </p>
                        ...
                        <table className="w-full" style={{ fontSize: f.xs }}>
                          <thead>
                            <tr className="text-left">
                              <th className="py-1.5 px-2" style={{ color: textDim }}>
                                列名
                              </th>
                              <th className="py-1.5 px-2" style={{ color: textDim }}>
                                类型
                          ...
                          <tbody>
                            {table.columns.map((col) => (
                              <tr key={col.name} className="border-t border-[rgba(0,180,255,0.04)]">
                                ...
                                <td className="py-1.5 px-2 text-[#7b8cff] font-mono">
                                  {col.dataType}
                                </td>
                                <td className="py-1.5 px-2">
                                  {col.nullable ? (
                                    <span className="text-[#ffaa00]">YES</span>
                                  ) : (
                                    <span style={{ color: textDim }}>NO</span>
                                  )}
                                </td>
                                <td className="py-1.5 px-2">
                                  {col.isPrimaryKey && <Key className="w-3 h-3 text-[#00ff88]" />}
                                </td>
                                <td className="py-1.5 px-2 font-mono" style={{ color: textDim }}>
                                  {col.defaultValue || "-"}
                                </td>
                              </tr>
                            ))}
                      ...
                      {db.tableDataLoading ? (
                        ...
                        <InlineEditableTable
                          ...
                          onCellChange={(_change: unknown, sql: string) => {
                            db.setSqlInput(sql);
                          }}
                        ...
                        <p className="mt-2" style={{ fontSize: f.xs, color: textDim }}>
                          无数据
                        </p>
                      )}
                    </div>
        ...
        <div className="space-y-3">
                    ...
                    .map((t) => (
                      ...
                      >
                        <span
                          ...
                          style={{ fontSize: f.xs, color: textPrimary }}
                        >
                          {t.label}
                        </span>
                        <span style={{ fontSize: "0.6rem", color: textDim }}>
                          {t.dbType === "all"
                            ? "通用"
                            : DB_TYPE_LABELS[t.dbType]?.label || t.dbType}
                        </span>
                      </button>
                    ))}
                </div>
          ...
          {db.queryResults.map((result) => (
            <GlassCard key={result.id} className="p-4">
              ...
              <div
                className="mb-2 px-2 py-1 rounded bg-[rgba(0,10,20,0.4)] font-mono text-[#7b8cff] overflow-x-auto"
                style={{ fontSize: f.xs }}
              >
                {result.sql}
              </div>
              ...
              {result.error ? (
                <p className="text-[#ff6464]" style={{ fontSize: f.sm }}>
                  {result.error}
                </p>
                  ...
                  <table className="w-full" style={{ fontSize: f.xs }}>
                    ...
                    <tbody>
                      {result.rows.map((row, ri) => (
                        <tr
                          key={ri}
                          className="border-b border-[rgba(0,180,255,0.03)] hover:bg-[rgba(0,40,80,0.15)]"
                        >
                          {result.columns.map((col) => (
                            <td
                              key={col}
                              className="py-1.5 px-2 text-[#c0dcf0] font-mono whitespace-nowrap max-w-[200px] truncate"
                            >
                              {String(row[col] ?? "NULL")}
                            </td>
                          ))}
      ...
      {activeTab === "history" && (
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <span style={{ fontSize: f.xs, color: textDim }}>
              {db.queryHistory.length} 条查询记录 (IndexedDB 持久化)
            </span>
            ...
            db.queryHistory.slice(0, 30).map((result) => (
              <GlassCard key={result.id} className="p-3">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span style={{ fontSize: f.xs, color: textDim }}>
                    {result.rowCount} 行 · {result.executionTimeMs.toFixed(1)}ms ·{" "}
                    {new Date(result.executedAt).toLocaleString("zh-CN")}
                  </span>
                  <button
                    onClick={() => {
                      db.replayQuery(result);
                      setActiveTab("query");
                    }}
                ...
                <pre
                  className="px-2 py-1 rounded bg-[rgba(0,10,20,0.4)] font-mono text-[#7b8cff] overflow-x-auto whitespace-pre-wrap"
                  style={{ fontSize: f.xs }}
                >
                  {result.sql}
                </pre>
              ...
              <GlassCard key={backup.id} className="p-4">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p style={{ fontSize: f.xs, color: textDim }}>
                      {backup.connectionName} · {formatBytes(backup.sizeBytes)} ·{" "}
