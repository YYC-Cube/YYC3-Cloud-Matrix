...
function getNetworkInfo(): { type: string; rtt: number } {
  const conn = (navigator as { connection?: { effectiveType?: string; rtt?: number } }).connection;
...
export function usePerformanceMonitor() {
  ...
  }, [state.current, state.vitals, state.history]); // eslint-disable-line react-hooks/exhaustive-deps
