...
<body>
  <h1>YYC³ Cloud Intelli-Matrix — ${data.title}</h1>
  <div class="subtitle">
  ...
  <div class="footer">
    YYC³ Cloud Intelli-Matrix — 自动生成报告 — ${new Date().toISOString().slice(0, 10)}
  </div>
...
export function useReportExporter() {
  ...
  const [initialReports, _setInitialReports] = useState<ReportHistoryEntry[]>(() => {
    const now = Date.now();
    return [
      { id: "rpt-prev-1", type: "performance" as const, time: now - 86400000, range: "24h" as const },
      { id: "rpt-prev-2", type: "security" as const, time: now - 86400000 * 2, range: "7d" as const },
      { id: "rpt-prev-3", type: "comprehensive" as const, time: now - 86400000 * 3, range: "24h" as const },
    ];
  });
