...
async function apiFallback<T>(
  ...
  try {
    ...
    return { ok: false, error: (err as { message?: string })?.message ?? "网络不可达" };
  }
}
...
export function useHostFileSystem() {
  ...
  const openDirectory = useCallback(async () => {
    ...
    try {
      ...
      if ((err as { name?: string })?.name !== "AbortError") {
        toast.error(`打开目录失败: ${(err as { message?: string })?.message}`);
      }
    } finally {
      setLoading(false);
    }
  ...
  const navigateToDir = useCallback(async (entry: HostFileEntry) => {
    ...
    try {
      ...
      toast.error(`无法打开: ${(err as { message?: string })?.message}`);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const navigateUp = useCallback(async () => {
    ...
    try {
      ...
      toast.error(`导航失败: ${(err as { message?: string })?.message}`);
    } finally {
      setLoading(false);
    }
  }, [currentPath]);
  ...
  const readFile = useCallback(async (entry: HostFileEntry) => {
    ...
    try {
      ...
      toast.error(`读取失败: ${(err as { message?: string })?.message}`);
      setEditingContent(null);
    }
  }, []);
  ...
  const saveFile = useCallback(async () => {
    ...
    try {
      ...
      toast.error(`保存失败: ${(err as { message?: string })?.message}`);
    }
  ...
  const createFile = useCallback(async (name: string, content = "") => {
    ...
    try {
      ...
      toast.error(`创建失败: ${(err as { message?: string })?.message}`);
    }
  ...
  const createDirectory = useCallback(async (name: string) => {
    ...
    try {
      ...
      toast.error(`创建目录失败: ${(err as { message?: string })?.message}`);
    }
  }, [getCurrentDirHandle, refreshCurrentDir]);
  ...
  const deleteEntry = useCallback(async (entry: HostFileEntry) => {
    ...
    try {
      ...
      toast.error(`删除失败: ${(err as { message?: string })?.message}`);
    }
  ...
  const renameEntry = useCallback(async (entry: HostFileEntry, newName: string) => {
    ...
    try {
      ...
      toast.error(`重命名失败: ${(err as { message?: string })?.message}`);
    }
  }, [getCurrentDirHandle, refreshCurrentDir]);
  ...
  const downloadFile = useCallback(async (entry: HostFileEntry) => {
    ...
    try {
      ...
      toast.error(`下载失败: ${(err as { message?: string })?.message}`);
    }
  }, []);
  ...
  const uploadFiles = useCallback(async (files: FileList | File[]) => {
    ...
    try {
      ...
      toast.error(`上传失败: ${(err as { message?: string })?.message}`);
    }
  }, [getCurrentDirHandle, refreshCurrentDir]);
  ...
  const restoreVersion = useCallback(async (version: FileVersion) => {
    ...
    try {
      ...
      toast.error(`恢复失败: ${(err as { message?: string })?.message}`);
    }
  }, [selectedEntry]);
